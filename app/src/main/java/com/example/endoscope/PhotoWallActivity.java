...
//缩略图显示图片
public class PhotoWallActivity extends Activity implements topbarClickListener,
	...
	private void init() {
		...
		VideoPath=folderPath.replace("/Image","/Video");
		...
		for (int i = allFileNames.length - 1; i >= 0; i--) {
			if (Utility.isImage(allFileNames[i])) {
				...
				bean = new BrowseListBean(allFileNames[i], imagePath);
				mListItems.add(bean);
			}
		}
		...
		// 设置Topbar中的文字信息
		mTopbar.setToptitle(getString(R.string.photo));
	...
	public void leftClick() {
		// TODO Auto-generated method stub
		Intent it = new Intent(PhotoWallActivity.this, BrowseActivity.class);
		it.putExtra("imagePath", folderPath);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		Intent it = new Intent(PhotoWallActivity.this, PhotoActivity.class);
...
public boolean onKeyDown(int keyCode, KeyEvent event) {
	...
	public void rightClick() {
		...
		select = mAdapter.getlist();
		if(select.size() > 0){
		dbHelper = new MyDatabaseHelper(PhotoWallActivity.this,
		...
		layout.setVisibility(View.INVISIBLE);
		mTopbar.setRightButtonIsVisiable(false);
		Intent intent=new Intent(PhotoWallActivity.this, Upload.class);
